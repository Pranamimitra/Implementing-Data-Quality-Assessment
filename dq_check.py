...
def remove_outliers_iqr(data, column):
    """Remove outliers using the IQR method. Handles only numeric columns."""
    if not pd.api.types.is_numeric_dtype(data[column]):
        st.warning(f"The column '{column}' is not numeric. Please select a numeric column.")
        return data  # Return original data if column is not numeric
...
if uploaded_file:
    ...
    if enable_visualization:
        ...
        if visualization_type == 'Histogram':
            ...
            if numeric_df.shape[1] > 5:
                st.warning("Pair plot is limited to 5 columns for better performance.")
    ...
    # Remove outliers
    if remove_outliers:
        column_to_check = st.sidebar.selectbox('Select Column for Outlier Removal', df.columns.tolist())
        if column_to_check:
